package hearsilent.kuas.food.particlesys;
...
import hearsilent.kuas.food.gl.Const;
import hearsilent.kuas.food.interfaces.Renderable;
...
public class ParticleSystem implements Renderable {
	...
	public ParticleSystem(ParticleSystemRenderer renderer, FloatBuffer vertexBuffer) {
		...
		glBufferData(GL_ARRAY_BUFFER, vertexBuffer.capacity() * Const.BYTES_PER_FLOAT, vertexBuffer,
				GL_STATIC_DRAW);
	...
	public void render() {
		final int stride =
				Const.BYTES_PER_FLOAT * (POS_DATA_SIZE + TEXTURE_COORDS_DATA_SIZE + MISC_DATA_SIZE);

		// a_Position
		glBindBuffer(GLES20.GL_ARRAY_BUFFER, mBufferId);
		GLES20.glEnableVertexAttribArray(mRenderer.positionHandle);
		GLES20.glVertexAttribPointer(mRenderer.positionHandle, POS_DATA_SIZE, GLES20.GL_FLOAT,
				false, stride, 0);

		// a_TexCoordinate
		glBindBuffer(GLES20.GL_ARRAY_BUFFER, mBufferId);
		GLES20.glEnableVertexAttribArray(mRenderer.textureCoordinateHandle);
		GLES20.glVertexAttribPointer(mRenderer.textureCoordinateHandle, TEXTURE_COORDS_DATA_SIZE,
				GLES20.GL_FLOAT, false, stride, Const.BYTES_PER_FLOAT * (POS_DATA_SIZE));

		// a_Misc
		glBindBuffer(GLES20.GL_ARRAY_BUFFER, mBufferId);
		GLES20.glEnableVertexAttribArray(mRenderer.miscHandle);
		GLES20.glVertexAttribPointer(mRenderer.miscHandle, MISC_DATA_SIZE, GLES20.GL_FLOAT, false,
				stride, Const.BYTES_PER_FLOAT * (POS_DATA_SIZE + TEXTURE_COORDS_DATA_SIZE));
