...
import hearsilent.kuas.food.gl.math.MathHelper;
...
public class ParticlesGenerator implements Runnable {
	...
	public void run() {
		final FloatBuffer vertexBuffer =
				makeInterleavedBuffer(generatePositions(), generateTextureUv(), generateMisc(),
	...
	private FloatBuffer makeInterleavedBuffer(float[] posData, float[] uvData, float[] miscData,
	                                          int numStars) {

		int dataLength = posData.length + uvData.length * numStars + miscData.length;

		final FloatBuffer interleavedBuffer =
				ByteBuffer.allocateDirect(dataLength * Const.BYTES_PER_FLOAT)
